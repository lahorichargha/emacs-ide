    ...
    (if eide-project-name
      ...
      (progn
        ...
        (if (not (string-equal (eide-config-get-project-value "debug_program_1") ""))
          (eide-i-popup-menu-add-action (concat "Debug (1): " (eide-project-get-full-gdb-command "debug_command_1" "debug_program_1")) "(eide-project-debug-1)" t))
        (if (not (string-equal (eide-config-get-project-value "debug_program_2") ""))
          (eide-i-popup-menu-add-action (concat "Debug (2): " (eide-project-get-full-gdb-command "debug_command_2" "debug_program_2")) "(eide-project-debug-2)" t))
...
(defun eide-popup-open-menu-for-file ()
  ...
  (save-excursion
    ...
    (if eide-config-show-svn-status-flag
      ...
      (setq l-buffer-svn-modified-flag nil)))
  ...
  (if eide-menu-grep-results-list
    (progn
      (dolist (l-grep-result eide-menu-grep-results-list)
        ...
        (eide-i-popup-menu-add-action l-grep-result (concat "(eide-search-view-result-buffer \"" l-grep-result-parameter "\")") t))
...
(defun eide-popup-open-menu-for-search ()
  ...
  (if eide-project-name
    (progn
      (eide-i-popup-menu-add-action "Go to definition (tag)" (concat "(eide-search-find-tag \"" string "\")") t)
      (eide-i-popup-menu-add-action "Find symbol (cscope)" (concat "(eide-search-find-symbol \"" string "\")") t)
